...
/**
 * The three stacked blocks (Flashcards / Reading / Listening) used by both
 * the standalone `DailyLoopSummaryCard` (rendered on /done) and the inline
 * `ProgressDayDetail` panel (rendered inside the calendar). Pure presentation;
 * no data-fetching, no headings, no footer — those belong to the surface that
 * embeds these blocks.
 */
export function DailyLoopSummaryBlocks({ summary }: { summary: DailyLoopSummary }) {
  return (
    <>
